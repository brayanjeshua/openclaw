...
const PROCESSED_IMAGE_MARKER_BYTES = Buffer.byteLength(
  JSON.stringify(PROCESSED_IMAGE_MARKER),
  "utf8",
);
...
export function fitWorkerReplayImages<T extends ReplayImageMessage>(
  ...
  let bytes = measureBytes(messages);
  ...
  for (let index = 0; index < observedBefore; index++) {
    ...
    for (const [blockIndex, block] of message.content.entries()) {
      ...
      // Replacing one JSON array element changes only its encoded block bytes.
      // Measure the full envelope once, then once more at the final boundary.
      bytes += PROCESSED_IMAGE_MARKER_BYTES - Buffer.byteLength(JSON.stringify(block), "utf8");
      if (bytes <= WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES) {
        return measureBytes(fitted) <= WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES
          ? fitted
          : undefined;
      }
    }
